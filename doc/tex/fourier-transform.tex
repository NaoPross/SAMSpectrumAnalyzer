...
\begin{figure}[H] \centering
	...
	\end{tikzpicture}
	\caption{
		Rappresentazione grafica della transformata di Fourier discreta \cite{ftplot}
		\label{fig:dftplot}
	}
\end{figure}
...
quest'ultimo.
\[
    c_k = \frac{1}{T}\int_0^T f(x)\cdot e^{-ik2\pi fx}\,\dd{x}
\]

Nella figura \ref{fig:dftplot}, possiamo osservare sul grafico \(x-z\) in rosso la
funzione campionata, ed il suo spettro discretto sull'asse \(y-z\) in blu.
...
I termini dei coseni \(a_k\) sono computati nel modo seguente.
\begin{align*}
...
I termini dei seni \(a_k\) sono computati nel modo seguente.
\begin{align*}
